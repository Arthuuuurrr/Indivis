...
execute if entity @e[tag=npc_leovic_registre_port,limit=1] if entity @e[type=marker,tag=wp_leovic_registre_port_a,limit=1] run tp @e[tag=npc_leovic_registre_port,limit=1] @e[type=marker,tag=wp_leovic_registre_port_a,limit=1]
scoreboard players set @e[type=armor_stand,tag=guide_leovic_registre_port] NPC_PATROL_STATE 0
...
tellraw @s [{"text":"[Guidage]","color":"gold","bold":true},{"text":" L’escorte de Léovic a été réinitialisée. Retournez lui parler pour reprendre la route.","color":"white"}]
