...
public class McCap {
    ...
    public static IntPtr Find(uint wanted) {
        ...
        EnumWindows((h, l) => {
            ...
            if (pid == wanted && IsWindowVisible(h) && GetWindowTextLength(h) > 0) { found = h; return false; }
            return true;
        }, IntPtr.Zero);
        return found;
    }
...
if (-not $proc) { Write-Output "NO_PROCESS pour pid $Pid_"; exit 1 }
$h = $proc.MainWindowHandle
if ($h -eq 0) {
    # MainWindowHandle est parfois vide pour un processus lance en arriere-plan : on enumere les
    # fenetres visibles appartenant a CE pid (jamais par titre, jamais une autre fenetre).
    $h = [McCap]::Find([uint32]$Pid_)
}
if ($h -eq 0) { Write-Output "NO_WINDOW pour pid $Pid_"; exit 1 }
